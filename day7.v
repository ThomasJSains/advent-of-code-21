...
Fixpoint _sort (output data : list N) :=
    match data with
    | x :: xs => _sort (insert x output) xs
    | [] => output
    end.

Definition sort := _sort [].

Fixpoint half (n : nat) := match n with
    | S (S n') => S (half n')
    | _ => n
    end.

Definition sorted_test_data := sort test_data.

Definition median (data : list N) := match (sort data) with sorted => nth (half (length sorted)) sorted 0 end.

Fixpoint sum_diffs (from : N) (data : list N) :=
    match data with
    | x :: xs => match (from <? x) with
        | true => (x - from) + sum_diffs from xs
        | false => (from - x) + sum_diffs from xs
        end
    | _ => 0
    end.

Definition part1 := sum_diffs (median data) data.
